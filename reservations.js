...
function updateSummary(){
  const total=guestCount();state.nights=nightCount(state.checkIn,state.checkOut);
...
function renderMonth(date){
  const year=date.getFullYear(),month=date.getMonth();const first=new Date(year,month,1),last=new Date(year,month+1,0);const weekdays=language==='es'?['D','L','M','M','J','V','S']:['S','M','T','W','T','F','S'];const cells=[];
  for(let i=0;i<first.getDay();i++)cells.push('<span class="calendar-day empty"></span>');
  for(let day=1;day<=last.getDate();day++){const d=new Date(year,month,day),iso=toIso(d);const classes=['calendar-day'];if(isPast(iso))classes.push('disabled');if(iso===state.checkIn)classes.push('selected','check-in');if(iso===state.checkOut)classes.push('selected','check-out');if(isInSelection(iso))classes.push('in-range');cells.push(`<button type="button" class="${classes.join(' ')}" data-date="${iso}" ${isPast(iso)?'disabled':''}>${day}</button>`)}
...
function renderCalendars(){if(!calendarMonths)return;if(!state.calendarStart){const now=new Date();state.calendarStart=new Date(now.getFullYear(),now.getMonth(),1)}const second=new Date(state.calendarStart.getFullYear(),state.calendarStart.getMonth()+1,1);calendarMonths.innerHTML=renderMonth(state.calendarStart)+renderMonth(second);calendarMonths.querySelectorAll('[data-date]').forEach(button=>button.addEventListener('click',()=>selectDate(button.dataset.date)))}
function selectDate(iso){if(!state.checkIn||state.checkOut||iso<state.checkIn){state.checkIn=iso;state.checkOut=''}else if(iso===state.checkIn){state.checkIn='';state.checkOut=''}else state.checkOut=iso;availabilityResult.hidden=true;dateMessage.textContent='';updateSummary();renderCalendars()}
...
document.querySelectorAll('[data-counter]').forEach(button=>button.addEventListener('click',()=>{const key=button.dataset.counter,change=Number(button.dataset.change);const total=guestCount();if(change>0&&total>=14)return;if(key==='adults')state.adults=Math.max(1,state.adults+change);if(key==='children')state.children=Math.max(0,state.children+change);updateSummary()}));

function goToStep(step){
  if(step<1||step>4||step>state.maxStep)return;state.step=step;document.body.dataset.bookingStep=String(step);
  ...
  if(mobileBookingBar){if(step===1){mobileBookingBar.textContent='Check availability';mobileBookingBar.style.display='flex'}else mobileBookingBar.style.display='none'}
...
document.getElementById('checkAvailabilityButton')?.addEventListener('click',()=>{updateSummary();if(!state.checkIn||!state.checkOut||state.nights<1){dateMessage.textContent='Select a check-in and check-out date to continue.';availabilityResult.hidden=true;return}dateMessage.textContent='';document.getElementById('availabilityTitle').textContent='Your stay selection is ready.';document.getElementById('availabilityText').textContent=`${formatDate(state.checkIn)} – ${formatDate(state.checkOut)} · ${state.nights} night${state.nights===1?'':'s'} · ${guestCount()} guest${guestCount()===1?'':'s'}. Live availability will be verified here once the public booking endpoint is connected.`;availabilityResult.hidden=false;availabilityResult.scrollIntoView({behavior:'smooth',block:'nearest'})});
...
document.getElementById('stripeButton')?.addEventListener('click',()=>{document.getElementById('paymentMessage').textContent='Stripe checkout will be connected here before launch. Frontend preview is continuing to confirmation.';state.maxStep=4;document.getElementById('confirmationGuest').textContent=[state.guest.firstName,state.guest.lastName].filter(Boolean).join(' ')||'Guest';document.getElementById('confirmationDates').textContent=state.checkIn&&state.checkOut?`${formatDate(state.checkIn)} – ${formatDate(state.checkOut)}`:'—';document.getElementById('confirmationGuests').textContent=`${guestCount()} guest${guestCount()===1?'':'s'}`;goToStep(4)});
...
renderCalendars();updateSummary();applyLanguage(language);document.body.dataset.bookingStep='1';
